...
export function Sidebar({ session, signOutAction }) {
    ...
    <div className="flex flex-col h-full bg-white dark:bg-white/5 backdrop-blur-xl border-r border-gray-200 dark:border-white/10 p-6">
      ...
      <div className="pt-6 border-t border-gray-200 dark:border-white/10 space-y-4">
          ...
          <div className="flex items-center gap-3 mb-2 px-2">
              {session?.user?.image && (
                  <img src={session.user.image} alt="User" className="w-8 h-8 rounded-full border border-gray-200 dark:border-white/20" />
              )}
          ...
          <form action={signOutAction}>
            <button className="flex w-full items-center gap-3 px-4 py-3 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-colors text-sm font-medium">
              <LogOut size={18} />
              Logout
            </button>
          </form>
