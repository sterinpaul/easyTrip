...
import { Plane, Car, PlaneTakeoff, PlaneLanding, Globe, Target, Landmark, Smartphone, MapPin, Phone, Edit2, Trash2, Printer, Download, Loader2, Train, Ship, Bus } from 'lucide-react';
...
const GuestCell = ({ label, value, highlight, colSpan }) => (
  <div className={`p-2.5 px-4 border-r border-b border-gray-200 dark:border-[#2a2a2a] ${colSpan ? colSpan : ''}`}>
          ...
          <div className="flex flex-col">
            ...
            {item.subActivities && item.subActivities.length > 0 && (
              <ul className="list-disc pl-5">
                {item.subActivities.map((subActivity, subIdx) => (
                  <li key={subIdx}>{subActivity}</li>
                ))}
              </ul>
            )}
          </div>
...
const getTransportIcon = (mode, isOutbound = true) => {
  const normalized = mode?.toLowerCase() || '';
  if (normalized.includes('car') || normalized.includes('cab')) return <Car className="w-10 h-10 text-[#c8a84b]" />;
  if (normalized.includes('bus')) return <Bus className="w-10 h-10 text-[#c8a84b]" />;
  if (normalized.includes('train')) return <Train className="w-10 h-10 text-[#c8a84b]" />;
  if (normalized.includes('ship') || normalized.includes('boat') || normalized.includes('cruise')) return <Ship className="w-10 h-10 text-[#c8a84b]" />;
  return isOutbound ? <PlaneTakeoff className="w-10 h-10 text-[#c8a84b]" /> : <PlaneLanding className="w-10 h-10 text-[#c8a84b]" />;
};
...
export default function ItineraryView({ itinerary }) {
  ...
  const onEdit = () => {
    router.push(`/itinerary/${itinerary._id || itinerary.id}/edit`);
  };
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const res = await fetch(`/api/itinerary/${itinerary._id || itinerary.id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  // Build destination segments title
  const segmentsTitle = destinations.map(d => d.name).join('-').toUpperCase() || 'TRAVEL ITINERARY';
  const segmentsList = destinations.map(d => d.name).join(', ');
  ...
  return (
    <div className={`font-['Segoe_UI',sans-serif] bg-[#f5f5f5] dark:bg-[#0a0a0a] text-gray-900 dark:text-white print:absolute print:left-0 print:top-0 print:w-full print:z-9999 print:bg-white print:text-black print:m-0 print:p-0 min-h-screen max-w-[900px] mx-auto p-0 ${bebasNeue.variable} ${montserrat.variable} transition-colors duration-300`}>
      ...
      <div className="print:hidden sticky top-0 z-50 bg-white/80 dark:bg-[#111]/80 backdrop-blur-md border-b border-gray-200 dark:border-[#222] p-4 mb-4 flex justify-between items-center rounded-b-lg shadow-sm">
        ...
        <div className="flex gap-2">
          <button onClick={onEdit} className="p-2 flex items-center gap-2 [font-family:var(--font-montserrat)] text-xs font-bold text-gray-600 dark:text-[#aaa] hover:text-[#c8a84b] dark:hover:text-[#c8a84b] hover:bg-gray-100 dark:hover:bg-[#1a1a1a] rounded transition-colors">
            <Edit2 className="w-4 h-4 sm:hidden inline" /> <span className="hidden sm:inline">EDIT</span>
          </button>
          <button onClick={onDelete} className="p-2 flex items-center gap-2 [font-family:var(--font-montserrat)] text-xs font-bold text-gray-600 dark:text-[#aaa] hover:text-red-500 hover:bg-red-50 dark:hover:bg-[#1a0a0a] rounded transition-colors ml-1">
            <Trash2 className="w-4 h-4 sm:hidden inline" /> <span className="hidden sm:inline">DELETE</span>
          </button>
          ...
          <button onClick={handlePrint} className="px-4 py-2 flex items-center gap-2 [font-family:var(--font-montserrat)] text-xs font-bold border border-[#c8a84b] text-[#c8a84b] hover:bg-[#c8a84b] hover:text-white dark:hover:text-black rounded transition-colors">
            <Printer className="w-4 h-4 sm:hidden inline" /> <span className="hidden sm:inline">PRINT</span>
          </button>
          <button onClick={handleDownloadPDF} disabled={isDownloading} className="px-4 py-2 flex items-center justify-center gap-2 [font-family:var(--font-montserrat)] text-xs font-bold bg-[#c8a84b] text-black hover:bg-[#b09442] rounded shadow-[0_2px_10px_rgba(200,168,75,0.3)] transition-colors disabled:opacity-70 disabled:cursor-not-allowed">
            {isDownloading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4 sm:hidden inline" />}
      ...
      <div ref={contentRef} className="print:bg-white print:text-black">
        <div className="pdf-page bg-white dark:bg-[#111] border-x border-t border-gray-200 dark:border-[#222]">
          ...
          <div className="relative flex flex-col justify-between gap-2 p-5 min-h-[400px] h-fit isolate print:h-[400px]">
            ...
            <h1 className="[font-family:var(--font-bebas)] text-5xl md:text-[80px] leading-[0.9] text-white/92 text-center drop-shadow-[2px_4px_20px_rgba(0,0,0,0.9)] tracking-[2px] md:tracking-[4px]">{segmentsTitle}</h1>

            <div className="bg-black/60 w-fit p-2 rounded-md z-10">
              <p className="[font-family:var(--font-montserrat)] text-[13px] font-semibold text-white/90 tracking-[1px]">
                {isProposal ? "PROPOSAL" : "ITINERARY"}{" "}
                <span className="bg-[#c8a84b] text-black [font-family:var(--font-montserrat)] font-bold text-xs rounded p-1 inline-block align-middle">
                  {itinerary?.duration?.nights || 0}N {itinerary?.duration?.days || 0}D
                </span>
              </p>
              {segmentsList && <p className="[font-family:var(--font-montserrat)] text-[11px] font-semibold text-white/90 tracking-[1px]">SEGMENTS: {segmentsList}</p>}
              {itinerary?.packageId && <p className="[font-family:var(--font-montserrat)] text-[11px] font-semibold text-white/90 tracking-[1px]">PACKAGE ID: {itinerary.packageId}</p>}
            </div>
          </div>

          {/* Guest Details */}
          <div className="grid grid-cols-2 bg-[#fafafa] dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#2a2a2a]">
            <GuestCell label="Guest Name" value={itinerary?.client?.name?.toUpperCase() || "GUEST"} highlight colSpan="col-span-2 md:col-span-1" />
            {isProposal && (
              <div className="grid grid-cols-2">
                <GuestCell label="Total Cost" value={formatCurrency(itinerary?.totalCost)} />
                <GuestCell label="Reward Points" value={itinerary?.rewardPoints || 0} colSpan="!border-r border-gray-200 dark:border-[#2a2a2a]" />
              </div>
            )}
            <GuestCell label="No of Guests" value={guestString(itinerary?.guestCount)} />
            <GuestCell label="Tour Duration" value={`${itinerary?.duration?.days || 0} DAYS`} />
            <GuestCell label="Departing From" value={itinerary?.departureFrom || ""} />
            <GuestCell label="Arriving At" value={itinerary?.arrivalAt || ""} />
            <GuestCell label="Tour Start Date" value={itinerary?.startDate ? new Date(itinerary.startDate).toLocaleDateString("en-GB") : ""} />
            <GuestCell label="Tour End Date" value={itinerary?.endDate ? new Date(itinerary.endDate).toLocaleDateString("en-GB") : ""} />
          </div>
          ...
          <div className="flex flex-col md:grid md:grid-cols-2 bg-[#fafafa] dark:bg-[#1a1a1a] border-x border-gray-200 dark:border-[#2a2a2a]">
            ...
            <div className="flex items-center gap-5 py-3 px-5 flex-wrap">
              <CheckOption label="HONEYMOON" checked={itinerary?.category === "HONEYMOON"} />
              <CheckOption label="FAMILY" checked={itinerary?.category === "FAMILY"} />
              <CheckOption label="GROUP" checked={itinerary?.category === "GROUP"} />
              <CheckOption label="SINGLE" checked={itinerary?.category === "SINGLE"} />
            </div>
          </div>
          ...
          <div className="flex flex-col md:grid md:grid-cols-2 bg-[#fafafa] dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#2a2a2a]">
            ...
            <div className="flex items-center gap-5 py-3 px-5 flex-wrap">
              <CheckOption label="ECONOMY" checked={itinerary?.type === "ECONOMY"} />
              <CheckOption label="STANDARD" checked={itinerary?.type === "STANDARD"} />
              <CheckOption label="PREMIUM" checked={itinerary?.type === "PREMIUM"} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 py-4 px-5">
            {(itinerary?.transportationModes || []).map((mode, i) => {
              const IconComp = TRANSPORT_ICONS[mode] || Plane;
              return (
                <div key={i} className="flex-1 border-2 border-[#c8a84b] rounded-lg flex items-center justify-center p-3.5 transition-colors">
                  <IconComp className="w-8 h-8 text-black dark:text-white" />
                </div>
              );
            })}
          </div>
            ...
            <div key={dIdx}>
              ...
              {destinations.length > 1 && (
                <div className="px-5 mt-4">
                  <div className="[font-family:var(--font-bebas)] text-[28px] tracking-[3px] text-[#c8a84b] border-l-4 border-[#c8a84b] pl-3">{dest.name?.toUpperCase()}</div>
                </div>
              )}

              {/* Travel Details */}
              {(dest.transportation?.outbound || dest.transportation?.inbound) && (
                <>
                  <div className="px-5">
                  ...
                  <div className="pdf-page m-[12px_20px_20px] print:break-inside-avoid">
                    <div className="border border-gray-200 dark:border-[#2a2a2a] rounded-lg overflow-hidden">
                      <div className="flex flex-col md:flex-row bg-white dark:bg-[#161616]">
                        {/* Outbound */}
                        {dest.transportation?.outbound && (
                          <div className="flex-1 flex items-center gap-4 p-4">
                            <div className="flex items-center justify-center shrink-0">
                              {getTransportIcon(dest.transportation.outbound.mode, true)}
                            </div>
                            <div className="[font-family:var(--font-montserrat)]">
                              <div className="text-lg font-extrabold text-gray-900 dark:text-white">{dest.transportation.outbound.from} → {dest.transportation.outbound.to}</div>
                              <div className="text-xs text-gray-600 dark:text-[#aaa] mt-0.5">{formatDate(dest.transportation.outbound.departureTime)}</div>
                              <div className="text-xl font-bold text-[#c8a84b] mt-1">
                                {formatTime(dest.transportation.outbound.departureTime)} → {formatTime(dest.transportation.outbound.arrivalTime)}
                              </div>
                              {dest.transportation.outbound.notes && (
                                <div className="text-[10px] text-gray-500 dark:text-[#888] mt-1">{dest.transportation.outbound.notes}</div>
                              )}
                            </div>
                          </div>
                        )}
                        ...
                        {/* Inbound */}
                        {dest.transportation?.inbound && (
                          <div className="flex-1 flex items-center gap-4 p-4">
                            <div className="flex items-center justify-center shrink-0">
                              {getTransportIcon(dest.transportation.inbound.mode, false)}
                            </div>
                            <div className="[font-family:var(--font-montserrat)]">
                              <div className="text-lg font-extrabold text-gray-900 dark:text-white">{dest.transportation.inbound.from} → {dest.transportation.inbound.to}</div>
                              <div className="text-xs text-gray-600 dark:text-[#aaa] mt-0.5">{formatDate(dest.transportation.inbound.departureTime)}</div>
                              <div className="text-xl font-bold text-[#c8a84b] mt-1">
                                {formatTime(dest.transportation.inbound.departureTime)} → {formatTime(dest.transportation.inbound.arrivalTime)}
                              </div>
                              {dest.transportation.inbound.notes && (
                                <div className="text-[10px] text-gray-500 dark:text-[#888] mt-1">{dest.transportation.inbound.notes}</div>
                              )}
                            </div>
                          </div>
                        )}
              ...
              {/* Hotel Details */}
              {dest.hotelDetails && dest.hotelDetails.length > 0 && (
                <>
                  <div className="px-5">
                  ...
                  <div className="m-[12px_20px_0] pb-5 print:break-inside-avoid">
                    {(() => {
                      // In itinerary mode, only show isSelected=true type; in proposal mode show all
                      const hotelTypes = isProposal
                        ? dest.hotelDetails
                        : dest.hotelDetails.filter(ht => ht.isSelected);

                      if (hotelTypes.length === 0) {
                        return (
                          <div className="border border-gray-200 dark:border-[#2a2a2a] rounded-lg overflow-hidden mb-3">
                            <div className="bg-white dark:bg-[#161616] p-6 text-center">
                              <div className="[font-family:var(--font-montserrat)] text-base font-bold text-gray-900 dark:text-[#aaa] tracking-[1px] opacity-70">
                      ...
                      return hotelTypes.map((ht, htIdx) => (
                        <div key={htIdx} className="border border-gray-200 dark:border-[#2a2a2a] rounded-lg overflow-hidden mb-3">
                          <div className="py-2.5 px-5 flex justify-between items-center bg-[#f9f9f9] dark:bg-[#111]">
                            <span className="ml-auto bg-[#c8a84b] text-black [font-family:var(--font-montserrat)] text-[11px] font-extrabold py-1 px-3 rounded tracking-[1px]">{ht.type}</span>
                          </div>
                          <div className="bg-white dark:bg-[#161616] p-5 text-center">
                            {(ht.hotels || []).map((hotel, hIdx) => (
                              <div key={hIdx}>
                                {hIdx > 0 && <div className="[font-family:var(--font-montserrat)] text-sm text-[#c8a84b] font-bold my-2">OR</div>}
                                <div className="[font-family:var(--font-montserrat)] text-base font-bold text-gray-900 dark:text-white tracking-[1px]">
                                  {typeof hotel === "object" ? hotel.name?.toUpperCase() : hotel}
                                  {typeof hotel === "object" ? `, ${hotel.city?.toUpperCase()}` : ""}
                                </div>
                              </div>
                            ))}
                          </div>
                    ...
                    <div className="[font-family:var(--font-montserrat)] text-[9px] text-gray-400 dark:text-[#555] text-right p-2 flex bg-[#f9f9f9] dark:bg-[#111]">
                      <span>SIMILAR PROPERTIES WILL BE PROVIDED IN CASE THE QUOTED PROPERTY IS UNAVAILABLE</span>
                    </div>
                  </div>
                </>
              )}
        ...
        <div className="pdf-page print:break-after-page bg-white dark:bg-[#111] border-x border-b border-gray-200 dark:border-[#222] px-6">
          ...
          {/* Render itinerary days from all destinations */}
          {destinations.map((dest, dIdx) => (
            <div key={dIdx}>
              {destinations.length > 1 && (
                <div className="[font-family:var(--font-montserrat)] text-sm font-black text-[#c8a84b] tracking-[2px] mt-4 mb-2 uppercase">{dest.name}</div>
              )}
              {(dest.itinerary || []).map((dayItem, dayIdx) => (
                <DayCard
                  key={dayIdx}
                  day={dayItem.day}
                  title={dayItem.title || `Day ${dayItem.day}`}
                  items={(dayItem.activities || [])}
                />
              ))}
            </div>
          ))}
          ...
          {((itinerary?.includes?.length > 0) || (itinerary?.excludes?.length > 0)) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-5 print:break-inside-avoid print:mt-4">
              {itinerary?.includes?.length > 0 && (
                <InfoCard title="INCLUDES THE FOLLOWING" items={itinerary.includes.filter(Boolean)} />
              )}
              {itinerary?.excludes?.length > 0 && (
                <InfoCard title="EXCLUDES THE FOLLOWING" items={itinerary.excludes.filter(Boolean)} />
              )}
            </div>
          )}

          {/* Trip Highlights */}
          {itinerary?.highlightImages && itinerary.highlightImages.length > 0 && (
            <div className="flex mb-5 p-4 bg-white dark:bg-[#161616] border border-gray-200 dark:border-[#2a2a2a] rounded-lg">
              ...
              <div className="flex gap-2 flex-1 items-center flex-wrap">
                {itinerary.highlightImages.map((img, i) => (
                  <div className="flex-1 min-w-[100px] h-[100px] rounded-lg overflow-hidden border-2 border-gray-200 dark:border-[#2a2a2a]" key={i}>
                    <img src={typeof img === "object" ? img.url : img} alt={`highlight ${i + 1}`} className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="print:break-inside-avoid [font-family:var(--font-montserrat)] text-[10px] text-gray-500 dark:text-[#888] mb-3 leading-[1.6]">
      ...
      <DeleteConfirmModal
        ...
        description="Are you sure you want to permanently delete this itinerary? This action cannot be undone."
      />
    </div>
  );
}
